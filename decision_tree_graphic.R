...
makebreakfast <- function(myrow, dim, mycolors){
  filename <- paste0("figures/waffleplot_tier", myrow["tier"] ,"_group", myrow["leaf"],"_", myrow["outcome"], ".pdf")
  print(filename)
...
colors_diagnosis <- c(my_colors["dark teal"],my_colors["dark yellow"],my_colors["dark red"], "white")
colors_sex <- c("red", "blue", "white")

my_waffles <- apply(mytable_diagnosis[which(mytable_diagnosis$tier %in% c("2","3")),], 1, makebreakfast, dim="square", mycolors=colors_diagnosis)
my_waffles <- apply(mytable_diagnosis[which(mytable_diagnosis$tier %in% c("1")),], 1, makebreakfast, dim="rectangle", mycolors=colors_diagnosis)
my_waffles <- apply(mytable_diagnosis[which(mytable_diagnosis$tier == "0"),], 1, makebreakfast, dim="custom", mycolors=colors_diagnosis)

my_waffles <- apply(mytable_sex[which(mytable_sex$tier %in% c("2","3")),], 1, makebreakfast, dim="square", mycolors=colors_sex)
my_waffles <- apply(mytable_sex[which(mytable_sex$tier %in% c("1")),], 1, makebreakfast, dim="rectangle", mycolors=colors_sex)
my_waffles <- apply(mytable_sex[which(mytable_sex$tier == "0"),], 1, makebreakfast, dim="custom", mycolors=colors_sex)
